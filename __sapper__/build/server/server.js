...
${($$result.head += `${($$result.title = `<title>blog | murakami naomi&#39;s portfolio site</title>`, "")}`, "")}

<h1>blog</h1>
<ul class="${"svelte-1frg2tf"}">${each(posts, post => `<li><a rel="${"prefetch"}"${add_attribute("href", post.fields.slug, 0)}>${escape(post.fields.title)} - ${escape(format(post.sys.createdAt, "YYYY/MM/DD"))} lo</a></li>`)}</ul>`;
});
